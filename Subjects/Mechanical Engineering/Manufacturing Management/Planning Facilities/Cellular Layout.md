...
![[Cellular Layout-Routing Matrix.png#align-right|Routing Matrix of the Facility|400]]
- Attempt to combine the flexibility of a process layout with the efficiency of a product layout. 
- Based on the concept of group technology (GT): 
  Dissimilar machines or activities are grouped into work centers, called cells, to process families of parts or customers with similar requirements. 
- The cells are arranged in relation to each other so that material movement is minimized. 
- Large machines that cannot be split among cells are located near to the cells that use them, that is, at their point of use.
- The Fig. shows the complete part routing matrix for the eight parts processed through a given facility. In its current form, there is no apparent pattern to the routings. 
